...
async fn main() {
    ...
    println!("NODE IDDDDDDDDDDDD {:?}", node_id);
    ...
    let launch_services = |logger: Logger| async move {
        ...
        println!("{:?}",opt);
        // Note: Opt { config: None, check_config: false, subgraph: None, postgres_url: Some("postgresql://graph-node:let-me-in@localhost:5432/graph-node"), postgres_secondary_hosts: [], postgres_host_weights: [], ethereum_rpc: ["testnet:https://data-seed-prebsc-1-s1.binance.org:8545/"], ethereum_ws: [], ethereum_ipc: [], ipfs: ["127.0.0.1:5001"], http_port: 8000, index_node_port: 8030, ws_port: 8001, admin_port: 8020, metrics_port: 8040, node_id: "default", debug: false, elasticsearch_url: None, elasticsearch_user: None, elasticsearch_password: None, ethereum_polling_interval: 1000, disable_block_ingestor: false, store_connection_pool_size: 10, network_subgraphs: [], arweave_api: "https://arweave.net/", three_box_api: "https://ipfs.3box.io/" }
        ...
        if !opt.disable_block_ingestor {
            // Note: BSC ethereum_polling_interval: 1000,
        ...
        // println!("subgraph instance manager {:?}", subgraph_instance_manager);
